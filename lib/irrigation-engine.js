...
function deriveEstimatedMoisture({ crop, soilType, weather, growthStage }) {
  const baseMoisture = SOIL_MOISTURE_BASELINES[soilType] || 55
  const cropTarget = crop?.optimalMoisture || 60
  const nextWeek = weather?.daily?.slice(0, 7) || []
  const avgTemp = nextWeek.length
    ? nextWeek.reduce((sum, day) => sum + ((Number(day.tempMax) + Number(day.tempMin)) / 2), 0) / nextWeek.length
    : Number(weather?.current?.temperature) || 28
  const totalRain = nextWeek.reduce((sum, day) => sum + (Number(day.rainSum) || 0), 0) + (Number(weather?.current?.rain) || 0)
  const stageEffect = growthStage === 'Flowering' || growthStage === 'Fruiting'
    ? -6
    : growthStage === 'Germination' || growthStage === 'Seedling'
      ? 4
      : 0
  const heatPenalty = avgTemp > 34 ? 8 : avgTemp > 30 ? 4 : 0
  const rainBoost = totalRain > 30 ? 10 : totalRain > 15 ? 5 : totalRain > 5 ? 2 : 0
  const estimated = (baseMoisture * 0.55) + (cropTarget * 0.45) + rainBoost - heatPenalty + stageEffect

  return clamp(round(estimated, 0), 28, 82)
}

export function buildSoilDataProfile({ crop, soilData, weather }) {
  const sensorsAvailable = soilData?.moisture !== null && soilData?.moisture !== undefined && soilData?.moisture !== ''
  const estimatedMoisture = deriveEstimatedMoisture({
    crop,
    soilType: soilData?.soilType || 'Loamy Soil',
    weather,
    growthStage: soilData?.growthStage || 'Vegetative',
  })

  return {
    moisture: sensorsAvailable ? Number(soilData.moisture) : estimatedMoisture,
    temperature: soilData?.temperature ?? weather?.current?.temperature ?? 25,
    ph: soilData?.ph ?? 6.5,
    ...
    sensorsAvailable,
    estimatedMoisture,
    moistureSource: sensorsAvailable ? 'sensor' : 'weather-estimated',
  }
}
...
export function computeIrrigationRecommendation({ crop, soilData, weather, fieldArea, location }) {
  ...
  const profiledSoil = buildSoilDataProfile({ crop, soilData, weather })
  const kc = getCropKc(cropName, profiledSoil.growthStage || 'Vegetative')
  const { TAW, depletionMM, soil } = soilWaterBalance(profiledSoil.soilType, cropName, profiledSoil.moisture)
  ...
  const totalNIR = nirLitersPerHa * fieldArea
  ...
  const needsWaterNow = depletionMM >= stressThreshold || profiledSoil.moisture < 30

  // Urgency
  let urgency = 'low'
  if (profiledSoil.moisture < 25) urgency = 'critical'
  else if (profiledSoil.moisture < 40 || depletionMM > RAW) urgency = 'high'
  else if (profiledSoil.moisture < 55) urgency = 'medium'
  ...
  const phPenalty = profiledSoil.ph < 5.5 || profiledSoil.ph > 8 ? 10 : 0
  const nPenalty = profiledSoil.nitrogen < 20 ? 5 : 0
  ...
  return {
    ...
    urgencyReason: urgency === 'critical' ? `Critical: Soil moisture ${profiledSoil.moisture}% is below wilting threshold`
      ...
      : `Soil moisture ${profiledSoil.moisture}% adequate for current stage`,
    ...
    waterAmountLiters: Math.round(totalNIR),
    durationMinutes: Math.round(totalNIR / (soil.infiltration * 60) / fieldArea),
    schedule,
    efficiencyScore,
    waterSaved: `${Math.round((waterSaved / traditionalUse) * 100)}% vs flood irrigation (${waterSaved.toLocaleString()}L saved)`,

    // Scientific analysis
    soilAnalysis: buildSoilAnalysis(profiledSoil, depletionMM, TAW, RAW),
    weatherImpact: buildWeatherImpact(weather, et0Values, etcPerDay, kc, rain7days),
    cropSpecificAdvice: buildCropAdvice(cropName, kc, profiledSoil.growthStage),
    ...
    moistureSource: profiledSoil.moistureSource,
    estimatedMoisture: profiledSoil.estimatedMoisture,

    // Alerts
    alerts: buildAlerts(profiledSoil, adjustedUrgency, rainExpected, rain3days),
    ...
    weeklyPlan: `Apply ${nirWeekMM.toFixed(0)}mm net (${Math.round(totalNIR).toLocaleString()}L) across ${schedule.length} sessions. ET₀=${avgET0.toFixed(1)}mm/day, ETc=${etcPerDay.toFixed(1)}mm/day`,
    fertigation: buildFertigationAdvice(profiledSoil),
  }
}
...
function generateSchedule({ needsWaterNow, rainExpected, totalNIR, nextWeek, bestTime }) {
  ...
  for (let i = 0; i < 7 && schedule.length < 3; i++) {
    ...
    const sessionLiters = totalNIR / (rainExpected ? 2 : 3)
    const adjustedLiters = Math.max(0, sessionLiters - (d?.rainSum || 0) * 10000 * 0.75)
    if (adjustedLiters < 50) continue
    schedule.push({
      day: dayName,
      date: new Date(today.getTime() + i * 86400000).toLocaleDateString('en-IN', { month: 'short', day: 'numeric' }),
      time: bestTime,
      liters: Math.round(adjustedLiters),
  ...
  if (schedule.length === 0) {
    ...
    schedule.push({ day: dayName, time: bestTime, liters: Math.round(totalNIR), rainForecast: 0, reason: 'Primary irrigation session' })
  }
...
export function generateWeeklyPlan({ crop, soilData, weather, fieldArea }) {
  const profiledSoil = buildSoilDataProfile({ crop, soilData, weather })
  const kc = getCropKc(crop.name, profiledSoil.growthStage || 'Vegetative')
  const today = new Date()
  ...
  return weather.daily?.slice(0, 7).map((d, i) => {
    ...
    return {
      dateKey: formatDateKey(new Date(today.getTime() + i * 86400000)),
      ...
      plannedLiters: Math.round(litersToApply),
      irrigate: irrigate,
...
export function summarizeIrrigationPattern(plan = [], crop, soilData) {
  const sessions = plan.filter(day => day.plannedLiters > 0)
  const totalLiters = sessions.reduce((sum, day) => sum + day.plannedLiters, 0)
  const averageSession = sessions.length ? Math.round(totalLiters / sessions.length) : 0
  const hottestDay = [...plan].sort((a, b) => Number(b.tempMax || 0) - Number(a.tempMax || 0))[0]

  return {
    sessionsPerWeek: sessions.length,
    totalLiters,
    averageSession,
    bestWatchDay: hottestDay?.date || 'This week',
    headline: sessions.length
      ? `Plan ${sessions.length} irrigation sessions this week, averaging ${averageSession.toLocaleString()}L each.`
      : 'No top-up is needed this week unless field conditions change.',
...
export function rebalancePlanWithLogs(plan = [], logs = [], fieldArea = 1) {
  const logsByDate = logs.reduce((acc, log) => {
    const key = formatDateKey(log.irrigated_at || log.created_at || new Date())
    acc[key] = (acc[key] || 0) + (Number(log.water_liters) || 0)
    return acc
  }, {})
  ...
  const enriched = plan.map(day => ({
    ...day,
    plannedLiters: Number(day.plannedLiters ?? day.liters ?? 0),
    adjustedLiters: Number(day.plannedLiters ?? day.liters ?? 0),
    actualLiters: Math.round(logsByDate[day.dateKey] || 0),
    carryOverLiters: 0,
  }))
  ...
  for (let i = 0; i < enriched.length; i += 1) {
    const day = enriched[i]
    const isPast = day.dateKey < todayKey

    if (isPast) {
      backlog += Math.max(0, day.plannedLiters - day.actualLiters)
      day.status = day.actualLiters >= day.plannedLiters && day.plannedLiters > 0 ? 'done' : day.plannedLiters > 0 ? 'missed' : 'hold'
      continue
    }

    day.carryOverLiters = Math.round(backlog)
    const todaysTarget = day.plannedLiters + backlog
    day.adjustedLiters = Math.round(todaysTarget)

    if (day.actualLiters > 0) {
      backlog = Math.max(0, todaysTarget - day.actualLiters)
      day.status = day.actualLiters >= todaysTarget ? 'completed' : 'partial'
    } else {
      backlog = Math.max(0, todaysTarget - day.plannedLiters)
      day.status = day.plannedLiters > 0 ? 'planned' : 'hold'
    }
  }

  return enriched.map(day => ({
    ...day,
    varianceLiters: Math.round(day.actualLiters - day.plannedLiters),
    recommendation: day.actualLiters > 0
      ? day.actualLiters >= day.adjustedLiters
        ? 'Completed for the day'
        : `Add ${Math.max(0, day.adjustedLiters - day.actualLiters).toLocaleString()}L later or carry it forward`
      : day.adjustedLiters > 0
        ? `Apply ${day.adjustedLiters.toLocaleString()}L`
        : 'No irrigation needed',
    depthMm: round((day.adjustedLiters || 0) / (10000 * fieldArea), 1),
  }))
}
